...
class DynamicSpreadZScoreStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if position_size == 0:  # 当前无持仓
            ...
            if z_score > self.p.entry_zscore:         # 价差大于 entry_zscore 个标准差 → 做空价差
                self._open_position(short=True)
            elif z_score < -self.p.entry_zscore:      # 价差小于 -entry_zscore 个标准差 → 做多价差
                self._open_position(short=False)
        else:  # 当前有持仓，考虑平仓
            if position_size > 0 and z_score > -self.p.exit_zscore:  # 做多价差的平仓条件
                self._close_positions()
            elif position_size < 0 and z_score < self.p.exit_zscore:  # 做空价差的平仓条件
                self._close_positions()
...
def run_strategy(data0, data1, data2, win, entry_zscore, exit_zscore, spread_window=60):
    ...
    # 添加策略
    cerebro.addstrategy(DynamicSpreadZScoreStrategy,
                        win=win,
...
def plot_heatmap(results, output_file='zscore_heatmap.png'):
    ...
    exit_zscores = sorted(df_results['params'].apply(lambda x: x['exit_zscore']).unique())
    spread_windows = sorted(df_results['params'].apply(lambda x: x['spread_window']).unique())
    ...
    for i, exit_zscore in enumerate(exit_zscores):
        ...
        # 过滤数据
        e_data = df_results[df_results['params'].apply(lambda x: x['exit_zscore']) == exit_zscore]
        
        for spread_window in spread_windows:
            # 进一步过滤特定的spread_window
            sw_data = e_data[e_data['params'].apply(lambda x: x['spread_window']) == spread_window]
...
def grid_search():
        ...
        for win in win_values:
            for entry_zscore in entry_zscore_values:
                for exit_zscore in exit_zscore_values:
                    param_combinations.append((data0, data1, data2, win, entry_zscore, exit_zscore, spread_window))
    ...
    for i, (data0, data1, data2, win, entry_zscore, exit_zscore, spread_window) in enumerate(param_combinations):
        print(f"测试参数组合 {i+1}/{total_combinations}: win={win}, entry_zscore={entry_zscore:.1f}, exit_zscore={exit_zscore:.1f}, spread_window={spread_window}")
        
        try:
            result = run_strategy(data0, data1, data2, win, entry_zscore, exit_zscore, spread_window)
    ...
    if results:
        ...
        print(f"入场Z-Score阈值 (entry_zscore): {best_result['params']['entry_zscore']:.2f}")
        print(f"出场Z-Score阈值 (exit_zscore): {best_result['params']['exit_zscore']:.2f}")
