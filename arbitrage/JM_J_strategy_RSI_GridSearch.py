...
class DynamicSpreadRSIStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if position_size == 0:  # 当前无持仓
            ...
            if rsi_value > self.p.overbought:         # RSI超买 → 做空价差
                self._open_position(short=True)
            elif rsi_value < self.p.oversold:         # RSI超卖 → 做多价差
                self._open_position(short=False)
        else:  # 当前有持仓，考虑平仓
            if position_size > 0 and rsi_value > self.p.exit_threshold:  # 做多价差的平仓条件
                self._close_positions()
            elif position_size < 0 and rsi_value < self.p.exit_threshold:  # 做空价差的平仓条件
                self._close_positions()
...
def run_strategy(data0, data1, data2, win, overbought, oversold, exit_threshold, spread_window=60):
    """运行单次回测"""
    # 创建回测引擎
    ...
    # 添加策略
    cerebro.addstrategy(DynamicSpreadRSIStrategy,
                        win=win,
...
def grid_search():
    ...
    # 定义参数网格
    win_values = [14, 20, 30]         # RSI计算窗口
    overbought_values = [70, 75, 80]  # 超买阈值
    oversold_values = [20, 25, 30]    # 超卖阈值
    exit_threshold_values = [45, 50, 55]  # 退出阈值
            ...
            for overbought in overbought_values:
                for oversold in oversold_values:
                    for exit_threshold in exit_threshold_values:
                        param_combinations.append((data0, data1, data2, win, overbought, oversold, exit_threshold, spread_window))
    ...
    for i, (data0, data1, data2, win, overbought, oversold, exit_threshold, spread_window) in enumerate(param_combinations):
        print(f"测试参数组合 {i+1}/{total_combinations}: win={win}, overbought={overbought}, oversold={oversold}, exit_threshold={exit_threshold}, spread_window={spread_window}")
        
        try:
            result = run_strategy(data0, data1, data2, win, overbought, oversold, exit_threshold, spread_window)
    ...
    if results:
        ...
        print(f"RSI窗口 (win): {best_result['params']['win']}")
        print(f"超买阈值 (overbought): {best_result['params']['overbought']}")
        print(f"超卖阈值 (oversold): {best_result['params']['oversold']}")
        print(f"退出阈值 (exit_threshold): {best_result['params']['exit_threshold']}")
        ...
        for i, result in enumerate(sorted_results[:10]):  # 只显示前10个最好的结果
            print(f"{i+1}. spread_window={result['params']['spread_window']}, "
                  ...
                  f"exit_threshold={result['params']['exit_threshold']}, "
