...
class ATRArbitrageStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 计算价差ATR
        self.price_diff_atr = bt.indicators.ATR(self.price_diff, period=self.p.atr_period)
        
        # 计算价差移动平均
        self.price_diff_ma = bt.indicators.SMA(self.price_diff, period=self.p.atr_period)
        
        # 计算上下轨
        self.upper_band = self.price_diff_ma + self.p.atr_multiplier * self.price_diff_atr
        self.lower_band = self.price_diff_ma - self.p.atr_multiplier * self.price_diff_atr
    ...
    def next(self):
        ...
        if self.position:
            ...
            if (self.position_type == 'long_j_short_jm' and 
                self.price_diff[0] >= self.price_diff_ma[0]):
                self.close(data=self.data0)
                self.close(data=self.data1)
                self.position_type = None
                if self.p.printlog:
                    print(f'平仓: 价差={self.price_diff[0]:.2f}, ATR={self.price_diff_atr[0]:.2f}')
            ...
            if self.price_diff[0] >= self.upper_band[0]:
                ...
                if self.p.printlog:
                    print(f'开仓: 做空J，做多JM, 价差={self.price_diff[0]:.2f}, ATR={self.price_diff_atr[0]:.2f}')
                ...
                if self.p.printlog:
                    print(f'开仓: 做多J，做空JM, 价差={self.price_diff[0]:.2f}, ATR={self.price_diff_atr[0]:.2f}')
